...
import type { ArchiveImportAction } from "../../types/depot";
import { isZipFileName } from "../../lib/depots/archiveImportUtils";

interface ArchiveImportDialogProps {
  file: File | null;
  path: string;
  onChoose: (action: ArchiveImportAction) => void;
  onCancel: () => void;
}

export function ArchiveImportDialog({ file, path, onChoose, onCancel }: ArchiveImportDialogProps) {
  if (!file) {
    return null;
  }
