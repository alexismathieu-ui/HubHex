...
import { emptyRepositoryDraft, type RepositoryDraft } from "../lib/repositoryUtils";

interface ProjectRepositoriesFieldProps {
  value: RepositoryDraft[];
  onChange: (repos: RepositoryDraft[]) => void;
  hint?: string;
}

export function ProjectRepositoriesField({ value, onChange, hint }: ProjectRepositoriesFieldProps) {
  const repos = value.length ? value : [emptyRepositoryDraft()];

  const updateAt = (index: number, patch: Partial<RepositoryDraft>) => {
