...
import type { RepositoryLink } from "../types/hubhex";

interface ProjectRepoListProps {
  repositories: RepositoryLink[] | undefined;
  compact?: boolean;
}

export function ProjectRepoList({ repositories, compact = false }: ProjectRepoListProps) {
  ...
  return (
    <ul className={`flex flex-col ${compact ? "gap-1.5" : "gap-2"}`}>
      {repositories.map((repo) => {
        const meta = providerMeta(repo.provider || "other");
        const name = repositoryDisplayName({
          label: repo.label,
          url: repo.url,
          provider: repo.provider as RepositoryDraft["provider"],
        });
