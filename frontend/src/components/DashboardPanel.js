...
export function DashboardPanel({ token, currentUser }) {
  ...
  if (!token || !currentUser) {
    return (
      <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
        <h3 className="text-xl font-semibold text-slate-200">Tableau de bord</h3>
        <p className="mt-2 text-sm text-slate-400">
          Connecte-toi pour voir le resume de tes projets, taches et activite.
        </p>
      </article>
    );
  }
  ...
  return (
    <article className="rounded-xl border border-amber-800/50 bg-amber-950/20 p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-xl font-semibold text-amber-200">Tableau de bord</h3>
